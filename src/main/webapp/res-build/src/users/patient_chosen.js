define(function (require, exports, module) {
    ...
    var listTpl = juicer(
        [
            ...
            '<p><a href="' + ROOTPAth + '/user/appointments/patientindex?patientid=${item.id}&patientname=${item.name}" class="btn btn-primary" role="button">历史预约记录</a>&nbsp;<a href="' + ROOTPAth + '/user/departmentqueues/index?patientid=${item.id}&patientname=${item.name}" class="btn btn-primary" role="button">队列当前就诊人序号</a><a href="' + ROOTPAth + '/user/hospitalizations/index?patientid=${item.id}&patientname=${item.name}" class="btn btn-primary" role="button">住院记录</a> </p>',
            '</div></div></div>',
            '{@/each}',
            '{@/if}'
        ].join(""));
