define(function (require, exports, module) {
    ...
    var listTpl = juicer(
        [
            ...
            '{@if item.status==1&&item.appointleftcount!=0}',
            '    <td><a class="btn btn-default btn-xs" style="color: green" href="' + ROOTPAth + '/admin/appointments/patientchosen?pcode=2&subcode=1&schedulingid=${item.id}">上班(${item.appointleftcount})</a></td>',
            '{@/if}',
            '{@if item.appointleftcount==0&&item.status==1}',
            '    <td style="color:#ffaf00;">号满</td>',
            '{@/if}',
            '{@if item.status==2}',
